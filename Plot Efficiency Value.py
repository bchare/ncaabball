...
# net_stats.loc[net_stats['team'].isin(["Charlotte","East Carolina","Fla. Atlantic","Memphis","North Texas","Rice","South Fla.","Temple","Tulane","Tulsa","UAB","UTSA","Wichita St."]), 'Conference'] = "AAC"
...
# net_stats.to_csv('estimated_net_output_conf.csv', index=False)
# net_stats.loc[net_stats['team'].isin([]), 'Conference'] = ""

# Input the real NET values
# https://stats.ncaa.org/selection_rankings/season_divisions/18403/nitty_gritties?utf8=%E2%9C%93&commit=Submit
realnet = pd.read_table('actual_net.txt')
realnet['NET'] = realnet['NET Rank']
realnet = realnet[['Team', 'NET', 'Conference']]
...
def makeplot(title, condition):
    ...
    fig.suptitle(('NET Rankings by Efficiency & Value Rating - ' + title + '\n(Games Through ' + maxdate + ')').strip())
...
# makeplot("Atlantic 10", "Conference == 'Atlantic 10'")
